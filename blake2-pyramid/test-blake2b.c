...
*/
	...
	for (length = 1; length <= BLAKE2B_OUTBYTES; length++) {
		memset(out, 0, BLAKE2B_OUTBYTES);
		ret = blake2b_init_key(S, length, key, BLAKE2B_KEYBYTES);
		ret = blake2b_update(S, data, length);
		ret = blake2b_final(S, out, length);

		printf("[%2d] %*s", length, BLAKE2B_KEYBYTES - length + 1, " ");
		for (i = 0; i < length; i++)
			printf("%02x", out[i]);
		printf("\n");
	}
